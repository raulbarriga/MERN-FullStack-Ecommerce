...
const authUser = asyncHandler(async (req, res) => {
    ...
    const user = await User.findOne({//find one document, & find it by:
...
const updateUserProfile = asyncHandler(async (req, res) => {
    ...
    const user = await User.findById(req.user._id)
    if(user) {//if it's in the request then it'll be updated by req.body.whatever, otherwise it stays the same/doesn't update
        user.name = req.body.name || user.name
        user.email = req.body.email || user.email
        //for password, first check if the password was sent:
        if(req.body.password) {//checking if the password was sent
...
const registerUser = asyncHandler(async (req, res) => {
    ...
    const user = await User.create({
        name,
        email,
        password//right now this oassword is plain text, but we'll get to the encryption part in a little bit
    })

    if (user) {
        //201 means something was created, 200 means it was a success
        res.status(201).json({//we send this data back, the same data from the login b/c we want to authenticate right after we register (log the person in when they create their new account)
            _id: user._id,
            name: user.name,
            email: user.email,
            isAdmin: user.isAdmin,
